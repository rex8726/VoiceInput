...
这是一个自用/小范围分享的工具，第一次用请注意三点：

1. **需要自备 API Key**：应用不内置任何 key。你需要自己注册并填入：
   - 硅基流动（语音转文字，必填）：<https://siliconflow.cn>
   - DeepSeek（文本润色，默认，便宜够快）：<https://platform.deepseek.com>；也可在设置里改用阿里百炼。
   打开菜单栏麦克风图标 →「设置...」，分别在「语音转文字」和「文本润色」区填入对应 Key。
2. **首次打开被拦是正常的**：应用是自签名、未做 Apple 公证，首次打开可能提示「无法验证开发者 / 已损坏」。解决办法：
   - 在 Finder 里**右键点 `VoiceInput.app` →「打开」**，在弹窗里再点「打开」；或
   - 到「系统设置 → 隐私与安全性」，页面下方点「仍要打开」。
   - 若提示「已损坏，无法打开」，在终端执行一次：`xattr -dr com.apple.quarantine /path/to/VoiceInput.app`
3. **需要授予权限**：麦克风（录音）、辅助功能（自动粘贴）。菜单栏和设置页会显示权限状态，也有「打开隐私设置」按钮。
